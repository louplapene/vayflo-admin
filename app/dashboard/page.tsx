...
async function getDashboardData() {
  ...
  return {
    ...
    totalImmeubles: totalImmeubles || 0,
    mrr,
  ...
  const stats = [
    {
      ...
      sub: `${data.totalTenants} total`,
      icon: Building2,
      color: "text-vayflo-600",
      bg: "bg-vayflo-50",
    },
    {
      label: "Utilisateurs",
      ...
      <div className="space-y-8">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Vue d&apos;ensemble de la plateforme ImmoGravity
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <div className="stat-card">
            <div className="mb-4 flex items-center gap-2">
              ...
              <h2 className="text-sm font-semibold text-gray-700">Factures en attente</h2>
            </div>
            {data.facturesEnAttente.length === 0 ? (
              ...
              <div className="space-y-3">
                {data.facturesEnAttente.map((f: { id: string; numero: string; montant_ht: number; date_echeance: string; statut: string }) => (
                  <div key={f.id} className="flex items-center justify-between rounded-lg bg-gray-50 px-3 py-2.5">
                    <div>
        ...
        <div className="stat-card">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-sm font-semibold text-gray-700">Derniers clients inscrits</h2>
            <a href="/clients" className="text-xs text-vayflo-600 hover:underline">Voir tous</a>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-50">
                {data.recentTenants.map((t: { id: string; nom: string; slug: string; actif: boolean; cree_le: string }) => (
                  <tr key={t.id} className="hover:bg-gray-50">
                    <td className="table-td">
                      <a href={`/clients/${t.id}`} className="font-medium text-gray-900 hover:text-vayflo-600">
                        {t.nom}
                      </a>
                    </td>
                    <td className="table-td text-gray-400 font-mono text-xs">{t.slug}</td>
                    <td className="table-td">
                      <span className={`badge ${t.actif ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}>
                        {t.actif ? "Actif" : "Inactif"}
                      </span>
                    </td>
                    <td className="table-td text-gray-400">{formatDate(t.cree_le)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
